...
` },
    ...
    /* ================================================================== */
    { t: 'insight', id: 'my-dma-motive', title: '★★ 这个 DMA 的动机是我自己推出来的', c: String.raw`
      接口那个缓冲区，不管是按字节还是再大一点，
      ==总之不是控制器里那么大的一块==。那 CPU 读外存里的数据时，是以字节为单位的吗？

      比如我指定一个地址，去读那个地址上的一个字节？==还是直接整块地读？==

      如果是整块地读，还要按中断方式去做 I/O 的话，
      ==是不是其实反而特别的浪费？==
    ` },

    { t: 'key', id: 'motive-confirmed', title: '这个判断完全正确，而且它就是教科书讲 DMA 动机时的标准论证', c: String.raw`
      而且==不是"反而"浪费，是本来就不可能这么干==。

      先把两套寻址方式分清楚——==混在一起才会觉得别扭==：

      | | 主存 | 外存（磁盘 / SSD） |
      |---|---|---|
      | 能不能按字节寻址 | ==能==，给个地址就返回那一个字节 | ==不能==，最小粒度是一个扇区 / 页 |
      | 物理原因 | 存储阵列本来就是按单元编址的 | ==磁头是"扫过"一段连续区域读出来的==，且 ECC 按扇区整体计算 |
      | 提请求的粒度 | "地址 $X$ 上那个字节" | ==只能说"第 $N$ 号扇区"== |

      所以"按字节读磁盘"这个请求==从接口层面就提不出来==。
      整块读进来是硬性的，剩下的问题就只是"这一块怎么搬"——
      ==而按字节中断搬，正是你算出来那个离谱的账。==

      **补一个容易忽略的后半段**：这 $512$ 字节被 DMA 搬进主存之后，
      它们就==变成普通的内存数据==了，
      CPU 这时候当然可以按字节随意访问其中任何一个——
      ==那已经是纯粹的访存，不再是 I/O 了==。
      "块"这个约束只存在于设备到主存这一段。
    ` },
