...
` },
    ...
    { t: 'warn', id: 'no-overlap', title: '★ 读一个主存块，几次访问之间能不能"叠"起来', c: String.raw`
      读一整块（比如 $32\ \text{B}$）要拆成好几次总线读。
      ==默认情况下这几次是完全独立、一次做完再做下一次的==：
      ...
      第 2 次:                            发地址 │ 主存准备 │ 传数据
                                          ██████████████████████████
      ★ 等主存准备的那几纳秒里，总线【空转但没让出来】，
        所以下一次的地址【钻不进来】。
      ~~~

      想让它们重叠，需要==题面额外给出机制==，而且不同机制省掉的东西不一样：

      | 想省掉什么 | 需要什么机制 | 效果 |
      |---|---|---|
      | 后几次的==发地址== | **突发传送** —— 一次地址，连传多个数据 | 地址开销只付一次 |
      | 让发地址==钻进上一次的等待里== | **[分离式总线事务](#/co/bus/bus-timing?at=split-why)** —— 等待期间释放总线 | 地址和传数被藏起来，==但每次 $6\ \text{ns}$ 的准备时间还在== |
      | 让主存==同时准备多个数据== | **多体交叉存储器** | 准备时间本身被并行掉 |

      ==这三个都要题面明说才能用==。题面一句"不支持突发传送"，
      连最基本的那条都堵死了 —— ==此时只能把单次读的时间原样重复 $n$ 遍==。

      **一句话记住**：==总线上的"流水线"不是默认功能，是要额外硬件买的。==
    ` },

    { t: 'example',
      id: 'ex-mem-block-read',
      title: '读一个主存块要多久（不支持突发传送）',
      level: 3,
      problem: String.raw`
        某存储器总线宽度为 $64$ 位，总线时钟频率为 $1\ \text{GHz}$，
        在总线上传输一个数据或地址需要一个时钟周期，==不支持突发传送方式==。
        若通过该总线连接 CPU 和主存，==主存每次准备一个 $64$ 位数据需要 $6\ \text{ns}$==，
        主存块大小为 $32\ \text{B}$，则读取一个主存块需要的时间为（　　）。

        **A.** $8\ \text{ns}$　　**B.** $11\ \text{ns}$　　**C.** $26\ \text{ns}$　　**D.** $32\ \text{ns}$
      `,
      idea: String.raw`
        这题只有两步，但两步都容易想多：

        **第一步：要读几次？** 总线一次搬 $64$ 位 $=8\ \text{B}$，
        块是 $32\ \text{B}$ $\Rightarrow$ ==$32\div8=4$ 次==。
        （$8\ \text{ns}$ 这个选项就是在这里收手的。）

        **第二步：一次多久？能不能重叠？**
        ==这一步是全题的分水岭==。看到"$6\ \text{ns}$ 准备时间"夹在中间，
        很自然会想到==流水线==：让下一次的地址钻进这 $6\ \text{ns}$ 里，
        算出 $8+6\times3=26\ \text{ns}$。

        这个直觉==不是凭空的==，它对应一个真实机制（[分离式总线事务](#/co/bus/bus-timing?at=split-why)）。
        但机制要==题面给==才能用。题面给的是反话：=="不支持突发传送"==，
        连"一次地址连传多个数据"都不行，更谈不上让总线在等待期间腾出来。

        ==所以这题的正确读法是：没有任何重叠，一次的时间乘以次数。==
      `,
      solution: String.raw`
        时钟周期 $T=\dfrac{1}{1\ \text{GHz}}=1\ \text{ns}$。

        **一次读取一个 $64$ 位数据**（三段严格串行）：

        | 阶段 | 时间 |
        |---|---|
        | 传地址 | $1$ 个时钟周期 $=1\ \text{ns}$ |
        | 主存准备数据 | $6\ \text{ns}$ |
        | 传数据 | $1$ 个时钟周期 $=1\ \text{ns}$ |
        | **合计** | $\mathbf{8\ \text{ns}}$ |

        **一个主存块要读几次**：

        $$\frac{32\ \text{B}}{64\ \text{位}} = \frac{32\ \text{B}}{8\ \text{B}} = 4\ \text{次}$$

        不支持突发传送 $\Rightarrow$ ==每一次都要重新发一遍地址，四次之间毫无重叠==：

        $$4 \times 8\ \text{ns} = 32\ \text{ns}\quad\Rightarrow\quad \text{选 D}$$
      `,
      comment: String.raw`
        **四个选项各自对应一台真实存在的机器** —— 这才是这道题好玩的地方：

        | 选项 | 它算的是哪种机器 | 算式 |
        |---|---|---|
        | **A. 8ns** | 只读了==一个 $64$ 位数据==就收手，忘了块是 $32\ \text{B}$ | $1+6+1$ |
        | **B. 11ns** | ==支持突发传送==的机器：一次地址，连传 4 个数据 | $1+6+4\times1$ |
        | **C. 26ns** | ==支持分离式事务、但主存仍是单体==的机器 | $8+6\times3$ |
        | **D. 32ns** | ✅ 本题：普通总线事务，==4 次完全独立地重复== | $4\times8$ |

        **$\mathbf{26}$ 这个数不是瞎凑的**，值得算清楚它成立的条件。
        若总线支持分离式事务（主设备发完地址==立刻让出总线==）：

        ~~~
        时刻  0    1         7    8        13   14        19  20       25  26
        地址  ─A1──A2─
        主存      ░░准备1░░░░│░░准备2░░░░│░░准备3░░░░│░░准备4░░░░
        数据                 ─D1─         ─D2─         ─D3─        ─D4─
        ★ 地址和传数都被藏进了等待里，但【单体主存一次只能准备一个】，
          6ns × 4 消不掉 ⇒ 1 + 6×4 + 1 = 26ns
        ~~~

        ==也就是说，$26$ 是"总线学会了流水，但主存没有"的答案==。
        要把那 $6\ \text{ns}$ 也叠起来，还得再加一个条件：==多体交叉存储器==。

        **回到考场上的判据**：

        > ==看到"不支持突发传送"，就把"重叠""流水""并行"全部划掉，
        > 老老实实算「单次时间 × 次数」。==
    ...
    { t: 'warn', id: 'pitfall-list', title: '这一节的固定失分点', c: String.raw`
      ...
      11. **默认几次总线读之间可以流水重叠** ——
          ==重叠是要题面给机制的==（突发 / 分离式事务 / 多体交叉），
          看到"不支持突发传送"就老实算「单次 $\times$ 次数」，
          见[这道题](#/co/bus/bus-basic?at=ex-mem-block-read)。
      12. **读一个"块"时忘了先算要读几次** ——
          ==次数 $=$ 块大小 $\div$ 总线一次能传的字节数==。
    ` },
