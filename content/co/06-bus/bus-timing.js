...
` },
    ...
    /* ================================================================== */
    { t: 'insight', id: 'my-latch-question', title: '★ 我自己看出来的：地址好像根本没被存起来', c: String.raw`
      我在内存读写、或者在 I/O 接口上从外设拿数据的时候，
      ==好像都没有发现有锁存地址的寄存器==，
      而是直接靠主设备把地址输出到地址线上，然后==把数据锁存到寄存器里==。

      是这样吗？
    ` },

    { t: 'key', id: 'addr-no-latch', title: '这个观察是对的：地址不用锁，数据必须锁', c: String.raw`
      两条线上发生的事，==本质不是一回事==：

      | | 地址线 | 数据线 |
      |---|---|---|
      | 有效多久 | ==整个总线周期都保持稳定==（主设备一直在驱动它） | ==只在周期中的一小段窗口内有效== |
      | 接收方拿它干什么 | 送进==地址译码器==——纯组合逻辑，实时译码 | 要==存下来==，供后续使用 |
      | 需要锁存吗 | ==不需要==（非复用总线） | ==必须==，在选通信号的有效沿打入 |

      **为什么地址不用锁**：译码器（$\texttt{74LS138}$ 那类）是==纯组合逻辑==，
      地址线上是什么电平，它就实时选中哪个单元；
      而地址在整个周期里==本来就没变过==，没有"抓一下"的必要。

      **为什么数据必须锁**：总线周期一结束，
      数据线==要么被别人驱动、要么直接进三态==，
      ==电平一晃就没了==。所以目标设备必须在 $\texttt{WR}$ / $\texttt{IOW}$ 的有效沿
      把它打进自己的寄存器。

      ==这正好解释了 I/O 接口里为什么有实打实的[数据缓冲寄存器](#/co/io/io-mode?at=interface-struct)，
      却没有"地址寄存器"==——地址只是过路信号，数据才是要留下来的东西。
    ` },

    { t: 'diagram', id: 'latch-window', title: '一个总线周期里，两条线的有效窗口不一样长',
      note: '地址是一条长横杠，数据只是中间一小段',
      caption: String.raw`==看的是"有效窗口"的长度==：地址线从头稳到尾，所以译码器随时取用都对；数据线只在中间一小段有效，所以必须靠==选通信号的边沿==把它抓下来。这也是[异步握手](#/co/bus/bus-timing?at=three-handshake)里那两根线真正在协调的东西——什么时候数据是可信的。`,
      svg: String.raw`
<svg class="dg" viewBox="0 0 700 186" role="img" aria-label="一个总线周期内地址线全程有效而数据线只在选通窗口内有效">
  <path class="gd" d="M130,20 V172"/>
  <path class="gd" d="M650,20 V172"/>
  <text class="cap" x="20" y="49">地址线</text>
  <g class="n k"><rect x="130" y="32" width="520" height="26" rx="5"/><text class="bt xs" x="390" y="45" text-anchor="middle" dominant-baseline="central">整个周期持续有效，译码器实时译码，不必锁存</text></g>
  <text class="cap" x="20" y="93">数据线</text>
  <g class="n a"><rect x="380" y="76" width="240" height="26" rx="5"/><text class="bt xs" x="500" y="89" text-anchor="middle" dominant-baseline="central">只在这一小段窗口内有效</text></g>
  <text class="cap" x="20" y="135">写选通 WR</text>
  <path class="wv" d="M130,122 H400 V140 H580 V122 H650"/>
  <path class="gd" d="M580,76 V152"/>
  <text class="lb em" x="646" y="164" text-anchor="end">↑ 在这个沿，数据才被打进目标寄存器</text>
</svg>
` },

    { t: 'warn', id: 'mux-needs-ale', title: '★ 例外：复用总线里，地址反而必须锁存', c: String.raw`
      上面那条结论有个==前提：地址线和数据线是物理分开的==。
      一旦[复用](#/co/bus/bus-basic?at=multiplex)，同一组引脚先送地址、后送数据，
      ==地址就必须先被抓下来==，否则引脚一切换，地址就没了。

      | 场景 | 靠什么锁 | 锁进哪 |
      |---|---|---|
      | 地址 / 数据复用总线 | $\texttt{ALE}$（地址锁存允许）信号 | 外部锁存器（经典是 $\texttt{74LS373}$） |
      | DRAM 的行列地址复用 | $\texttt{RAS}$ / $\texttt{CAS}$ 两个信号分别锁 | ==片内==的行地址锁存器、列地址锁存器 |

      ==所以"要不要锁存地址"完全取决于线是不是复用的==，
      不是什么普遍规律。选择题里看到"$\texttt{ALE}$"或"$\texttt{74LS373}$"，
      基本就是在暗示==这是一条复用总线==。
    ` },
