...
` },
    ...
    { t: 'example', id: 'ex-merge-order',
      title: '★★ 六个不等长有序表，怎么合并才最省比较次数',
      source: '统考真题',
      level: 4,
      problem: String.raw`
        设有 6 个有序表 $A,B,C,D,E,F$，分别含有 $10,\ 35,\ 40,\ 50,\ 60,\ 200$ 个数据元素，
        各表中元素按升序排列。要求通过 ==5 次两两合并==，将 6 个表最终合并成 1 个升序表，
        并==在最坏情况下比较的总次数达到最小==。请回答：
      ...
      idea: String.raw`
        **先把单次合并的代价定下来**：
        长度为 $m$ 和 $n$ 的两个有序表归并，==最坏情况下比较 $m+n-1$ 次==
        （每比较一次至少确定一个元素的位置，最后一个元素不需要再比）。

        既然代价正比于"两表长度之和"，那么==总代价就正比于每个表"参与合并的次数"×"它的长度"==
        —— ==这正是[带权路径长度 WPL](#/ds/tree/huffman?at=wpl-def)==。

        于是问题变成：==以各表长度为叶子权值，构造一棵 2 叉哈夫曼树==。
        做法就是[哈夫曼算法](#/ds/tree/huffman?at=build-steps)：==每次取当前最短的两个表合并==。

        ==手算时把长度排成一行、每合并一次就把新值按序插回去==，
        这样下一轮还是取最左边两个，不用重新扫。
      `,
      solution: String.raw`
        初始：$A(10),\ B(35),\ C(40),\ D(50),\ E(60),\ F(200)$

        **(1) 合并过程**

        | 步 | 当前集合（升序） | 取出的两个 | 新表长度 | 最坏比较次数 |
        |---|---|---|---|---|
        | 1 | 10, 35, 40, 50, 60, 200 | $A(10),B(35)$ | $N_1=45$ | $10+35-1=44$ |
        | 2 | 40, **45**, 50, 60, 200 | $C(40),N_1(45)$ | $N_2=\mathbf{85}$ | $40+45-1=84$ |
        | 3 | 50, 60, **85**, 200 | $D(50),E(60)$ | $N_3=110$ | $50+60-1=109$ |
        | 4 | 85, **110**, 200 | $N_2(85),N_3(110)$ | $N_4=195$ | $85+110-1=194$ |
        | 5 | **195**, 200 | $N_4(195),F(200)$ | $N_5=395$ | $195+200-1=394$ |

        $$\text{总比较次数}=44+84+109+194+394=\boxed{825}$$

        **(2) 合并策略与理由**

        **策略**：==每次从当前的有序表集合中，选取长度最短的两个表进行合并，
        把合并后生成的新表放回集合==；重复此操作，直到集合中只剩一个表。

        **理由**：
        1. ==这等价于构造一棵最佳归并树（2 叉哈夫曼树）==：
           把每个表的长度看作叶子的权值，一次合并的代价 $m+n-1$
           与两表长度之和（即哈夫曼树中该内部结点的权值）只差一个常数 1；
        2. ==由哈夫曼树的构造原理，每次合并权值最小的两个结点，
           能让权值大的叶子尽量靠近根==，
           从而使==长表参与合并的次数最少==，
           所有内部结点权值之和（即 WPL）达到最小，总比较次数也随之最小。
      `,
      comment: String.raw`
        **一个非常好用的交叉验算**：

        $$\text{总比较次数}=\underbrace{\sum(\text{各内部结点权值})}_{=\ \mathrm{WPL}}-\ (\text{合并次数})$$

        本题内部结点权值之和 $=45+85+110+195+395=830$，合并了 5 次，
        故 $830-5=825$ ✓

        再用 WPL 的定义验一遍（根的深度记为 0）：
        $F$ 在第 1 层，$C,D,E$ 在第 3 层，$A,B$ 在第 4 层：
        $$\mathrm{WPL}=200\times 1+40\times 3+50\times 3+60\times 3+10\times 4+35\times 4=830\ \checkmark$$

        ==两条路算出同一个 830，基本可以确定没错==。

        **最容易出事的地方不是策略，而是加法**。
        第 2 步 $40+45=85$，若手滑写成 $95$，==后面四步的数字会全部连锁出错==，
        而策略分照拿、计算分全丢。
        建议==每算出一个新长度就立刻和"当前集合的总和不变"对一次账==：
        任何时刻集合里所有表的长度之和恒为 $10+35+40+50+60+200=395$。
        第 2 步后集合是 $\{50,60,85,200\}$，和为 $395$ ✓

        **和[最佳归并树那一节](#/ds/sort/external?at=best-idea)的关系**：
        本题就是 $k=2$ 的特例。$k$ 路时改成"每次取最小的 $k$ 个"，
        并且==要先检查是否需要补[虚段](#/ds/sort/external?at=virtual-seg)==；
        $k=2$ 时 $(r-1)\bmod(k-1)=(6-1)\bmod 1=0$ 恒成立，==2 路永远不用补虚段==。
      `,
    },
