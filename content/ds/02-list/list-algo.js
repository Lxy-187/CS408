...
` },
    ...
    { t: 'insight', id: 'note-two-pass', title: '我第一次写的是两趟法 —— 它错在哪', c: String.raw`
      我当时的设计思想是：

      > ==遍历链表，计算出链表的长度，得到倒数第 $k$ 个元素的正向位序；
      > 再次遍历链表，找到该位序的元素返回。==

      这个思路==逻辑上完全正确，复杂度也是 $O(n)$ / $O(1)$==，
      但拿到这道题上有三处会被扣分：

      1. ==**"尽可能高效"没做到**==。两趟法遍历了链表两次；
         而本题存在==一趟==就能解决的双指针法。
         题面里"尽可能高效"这五个字==就是在暗示存在更优解==，看到它就要往"一趟"上想。
      2. ==**返回值不符合题意**==。题目要求"输出该结点的 data 值，**并返回 1**；否则返回 0"。
         我直接 $\texttt{return ptr->data}$ 了，==既没有 $\texttt{printf}$，返回值语义也错了==。
      3. ==**头结点没剥掉**==。我让指针从 $\texttt{list}$（头结点）开始计数，
         算出来的 $\texttt{count}$ 把头结点也算进去了，==后面的位序运算极易差 1==。

      **给自己的三条自查**：
      ==看到"尽可能高效"先问能不能一趟==；
      ==返回值严格照抄题面的措辞==；
      ==带头结点的题，指针一律从 $\texttt{list->next}$ 起步==。
    ` },
    ...
    { t: 'example', id: 'ex-common-suffix',
      ...
      comment: String.raw`
        ...
        **另一种写法见[下面的"轨迹切换法"](#/ds/list/list-algo?at=swap-track)** ——
        它更短、更巧，但边界极难写对，==考场上建议写上面的标准解法==。
    ...
    { t: 'warn', id: 'swap-track', title: '★ 轨迹切换法：思路很巧，但三个边界极易写错', c: String.raw`
      **思路**：让 $p$ 走完 $\texttt{str1}$ 之后接着走 $\texttt{str2}$，
      $q$ 走完 $\texttt{str2}$ 之后接着走 $\texttt{str1}$。
      ==两者各走了 $m+n$ 步，路程完全相同，所以必然在交点相遇==；
      若没有交点，则==同时走到 $\texttt{NULL}$ 而退出==。

      听起来很干净，但手写时有三个坑，==每一个都足以让代码彻底跑不起来==：

      1. ==**$\texttt{return}$ 写进了循环体里**==。
         循环第一次执行完就返回了，==根本没开始遍历==。
         $\texttt{return}$ 必须在 $\texttt{while}$ **外面**。
      2. ==**切换时跳过了 $\texttt{NULL}$，导致死循环**==。
         若写成"$\texttt{if (p->next != NULL) p = p->next; else p = str2;}$"，
         指针==永远不会取到 $\texttt{NULL}$==；
         两条链没有公共后缀时，$p$ 和 $q$ ==永远无法同时为空，循环停不下来==。
         正确做法是==让指针先走到 $\texttt{NULL}$，下一步再切换==。
      3. ==**没剥掉头结点**==。题目是带头结点的链表，
         两个头结点是不同的两个结点，==必须从 $\texttt{str1->next}$ / $\texttt{str2->next}$ 起步==。

      **改对之后的写法**：

      ~~~c
      Node *FindAddr2(Node *str1, Node *str2) {
          Node *p = str1->next;                 // 从首元结点开始
          Node *q = str2->next;
          while (p != q) {                      // 相等时退出（交点，或同时为 NULL）
              p = (p == NULL) ? str2->next : p->next;   // 先允许取到 NULL，再切换
              q = (q == NULL) ? str1->next : q->next;
          }
          return p;                             // ← return 在循环外
      }
      ~~~

      ==为什么它一定会停==：设两条链在交点前分别长 $a$、$b$，公共后缀长 $c$。
      $p$ 走 $a+c+b$ 步到达交点，$q$ 走 $b+c+a$ 步到达交点，==步数相同==。
      无交点时 $c=0$，两者都在第 $a+b$ 步同时变成 $\texttt{NULL}$，==条件 $p==q$ 成立==，退出。
    ` },
